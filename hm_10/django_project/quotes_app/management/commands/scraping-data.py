...
class Command(BaseCommand):
    help = "Data scraping from website"
...
class QuotesPipeline:
    ...
    def close_spider(self, spider):
        with open("scraped_data/quotes.json", "w", encoding="UTF-8") as fd:
            json.dump(self.quotes, fd, ensure_ascii=False)
        with open("scraped_data/authors.json", "w", encoding="UTF-8") as fd:
...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response, *args):
        def parse_quotes(field):
            ...
            return QuoteItem(tags=tags, author=author, quote=quote)
        ...
        def parse_authors(author_response):
            ...
            description = (
                field.xpath("div[@class='author-description']/text()")
                .get()
                .strip()
                .replace('"', "")
            )
            born_date = field.xpath("p/span[@class='author-born-date']/text()").get()
            born_location = field.xpath(
