...
Send a confirmation email for email verification.
    ...
    try:
        ...
        fm = FastMail(conf)
        await fm.send_message(message, template_name="register/confirm.html")
    except ConnectionErrors as err:
        print(err)


async def send_reset_email(email: EmailStr, username: str, host: str, password_reset_link: str):
    """
Send a password reset email.
    ...
    :type host: str
    """
    message_body = f"For password reset click on the link: {password_reset_link}"
    try:
        ...
        await fm.send_message(
            message, template_name="reset_password/password_reset_email.html"
        )
    except ConnectionErrors as err:
        print(err)
