...
async def create_contact(
    contact: ContactRequest, db: Session, current_user: User
) -> Contact:
    """
...
async def get_contact(
    contact_id: int, db: Session, current_user: User
) -> Type[Contact]:
    """
Retrieve a contact by its ID for the given user.
    :param contact_id: The ID of the contact to retrieve.
    :type contact_id: int
    :param db: The SQLAlchemy database session to use for the query.
    :type db: Session
    :param current_user: The authenticated user for whom the contact is being retrieved.
    :type current_user: User
    :return: The Contact object corresponding to the given `contact_id` if found.
    :rtype: Type[Contact]
    :raises HTTPException 404: If the contact is not found.
    """
    contact = (
        db.query(Contact)
        .filter(and_(Contact.id == contact_id, Contact.user_id == current_user.id))
        .first()
    )
    if contact is None:
...
async def get_contacts(
    skip: int, limit: int, db: Session, current_user: User
) -> list[Type[Contact]]:
    """
...
async def search_contacts(
    q: str, skip: int, limit: int, db: Session, current_user: User
) -> list[Type[Contact]]:
    """
